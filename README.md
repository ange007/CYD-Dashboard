...
> Submodules (`boards/`, `companion/`) are required — don't forget `--recurse-submodules`.
...
The desktop **companion** ([companion/](companion) submodule, Go) relays HID input from a host PC and supplies system metrics to widgets — useful where BLE isn't available or for richer host integration.
