...
/**
 * Mutex — recursive FreeRTOS mutex. enable/disable wrap cross-task UI
 * mutations; LVGL refresh uses _ui_try_lock()/_ui_unlock() with a 0-tick
 * timeout to skip frames during mutations instead of racing them.
 * The priority argument is retained for API compatibility (legacy callers
 * pass 1 or 2) but no longer gates take/give — recursion handles nesting.
 */

void _ui_mutex_init(void);
void _ui_enable_mutex(int priority);
void _ui_disable_mutex(int priority);
bool _ui_get_mutex(void);
...
#ifdef __cplusplus
// RAII guard — takes recursive UI mutex in ctor, releases in dtor.
// Use in C++ code instead of _ui_enable_mutex / _ui_disable_mutex pairs.
struct UiLock {
    UiLock()  { _ui_enable_mutex(1); }
    ~UiLock() { _ui_disable_mutex(1); }
};
#endif
