<template>
  <div class="page">
    <div class="page-header">
      <h2>Widgets</h2>
      <div class="toolbar">
        <RouterLink to="/scene/widgets/new" class="btn btn-ghost">+ Add scene</RouterLink>
        <button class="btn btn-ghost" @click="showTemplates = true">From template</button>
        <RouterLink to="/widgets/new" class="btn">+ Add widget</RouterLink>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div class="tree-section">
        <div class="tree-header tree-header-root">
          ...
          <span class="text-muted" style="font-size:11px">{{ rootWidgets.length }} widget(s)</span>
          <div class="tree-header-actions">
            <button class="btn btn-ghost btn-sm" title="Root background &amp; re-optimize" @click="openRootPopup">🖼 BG</button>
          </div>
        </div>
        <table v-if="rootWidgets.length" class="table tree-table">
          <tbody>
            <tr v-for="(w, idx) in rootWidgets" :key="w.id">
              <td class="order-cell">
                <button class="sort-btn" :disabled="idx === 0" @click="store.moveInScene(w.id, -1)" title="Move up">▲</button>
                <button class="sort-btn" :disabled="idx === rootWidgets.length - 1" @click="store.moveInScene(w.id, 1)" title="Move down">▼</button>
              </td>
              <td>
                <span v-if="w.bg_color != null" class="color-dot" :style="{ background: '#' + w.bg_color.toString(16).padStart(6,'0') }"></span>
                {{ w.title || '—' }}
              </td>
              <td class="type-cell">{{ w.type }}{{ styleLabel(w) }}</td>
              <td class="src-cell text-muted">{{ w.data_target }}{{ w.url ? ` → ${w.url}` : '' }}</td>
              <td class="refresh-cell">{{ w.update ? `${w.update}s` : 'manual' }}</td>
              <td class="actions-cell">
                <select class="scene-assign" :value="w.scene_id ?? ''" @change="onAssign(w.id, ($event.target as HTMLSelectElement).value)" title="Move to scene">
                  <option value="">Root</option>
                  <option v-for="s in sceneItems" :key="s.id" :value="s.id">📁 {{ s.title }}</option>
                </select>
                <RouterLink :to="`/widgets/${w.id}`" class="btn btn-ghost btn-sm">Edit</RouterLink>
                <button class="btn btn-danger btn-sm" @click="onDelete(w.id)">Del</button>
              </td>
            </tr>
          </tbody>
        </table>
        <div v-else class="tree-empty">No root-level widgets. Click <strong>+ Add widget</strong>.</div>
      </div>
      ...
      <div v-for="scene in sceneItems" :key="scene.id" class="tree-section">
        <div class="tree-header tree-header-scene">
          ...
          <span class="text-muted" style="font-size:11px">{{ widgetsInScene(scene.id).length }} widget(s)</span>
          <div class="tree-header-actions">
            <RouterLink :to="`/scene/widgets/${scene.id}`" class="btn btn-ghost btn-sm">Edit scene</RouterLink>
            <button class="btn btn-danger btn-sm" @click="onDeleteScene(scene.id)">Delete scene</button>
          </div>
        </div>
        <table v-if="widgetsInScene(scene.id).length" class="table tree-table">
          <tbody>
            <tr v-for="(w, idx) in widgetsInScene(scene.id)" :key="w.id">
              <td class="order-cell">
                <button class="sort-btn" :disabled="idx === 0" @click="store.moveInScene(w.id, -1)" title="Move up">▲</button>
                <button class="sort-btn" :disabled="idx === widgetsInScene(scene.id).length - 1" @click="store.moveInScene(w.id, 1)" title="Move down">▼</button>
              </td>
              <td>
                <span v-if="w.bg_color != null" class="color-dot" :style="{ background: '#' + w.bg_color.toString(16).padStart(6,'0') }"></span>
                {{ w.title || '—' }}
        ...
        <div v-else class="tree-empty">No widgets in this scene yet.</div>
      </div>

      <div v-if="!store.items.length" class="card text-muted" style="margin-top:16px">
        No widgets yet. Click <strong>+ Add widget</strong> or <strong>From template</strong>.
      </div>
    </template>
  </div>
    ...
    <div v-if="rootPopupOpen" class="modal-backdrop" @click.self="rootPopupOpen = false">
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div v-if="rootBgFiles.length === 0 && !rootBg" class="text-muted" style="font-size:12px;margin-bottom:8px">
            No background images on SD card yet.
          </div>
          <div v-if="rootBgFiles.length > 0 || rootBg" class="icon-gallery" style="margin-bottom:8px">
          ...
          <p v-if="rootBgSaveError" class="text-danger" style="font-size:12px;margin-bottom:4px">{{ rootBgSaveError }}</p>
          ...
          <div class="hint-box" style="margin-top:8px">
            Rebuilds <em>_s/_m/_l</em> variants for icons used by root-level widgets.
    ...
    <div v-if="showTemplates" class="modal-backdrop" @click.self="showTemplates = false">
      <div class="modal">
        ...
        <div class="tpl-filter-bar">
          ...
          <button
            v-for="cat in TEMPLATE_CATEGORIES" :key="cat" type="button"
            :class="['tpl-filter-pill', { active: activeCategories.has(cat) }]"
            @click="toggleCategory(cat)"
          >{{ cat }}</button>
        </div>
        <div class="modal-body">
          <template v-for="cat in TEMPLATE_CATEGORIES" :key="cat">
...
import { WIDGET_TEMPLATES, TEMPLATE_CATEGORIES, type WidgetTemplate } from '../utils/widgetTemplates';
...
const activeCategories = ref(new Set<string>(TEMPLATE_CATEGORIES));

function toggleCategory(cat: string) {
  const s = activeCategories.value;
  if (s.has(cat)) s.delete(cat); else s.add(cat);
  activeCategories.value = new Set(s);
}
function selectAllCategories()   { activeCategories.value = new Set(TEMPLATE_CATEGORIES); }
function deselectAllCategories() { activeCategories.value = new Set(); }

const sceneItems = computed(() =>
  store.items.filter(w => w.type === 'scene' && !w.scene_id)
);

const rootWidgets = computed(() =>
  store.items.filter(w => !w.scene_id && w.type !== 'scene')
);

function widgetsInScene(sceneId: string) {
  return store.items.filter(w => w.scene_id === sceneId && w.type !== 'scene');
}

function styleLabel(w: Widget) {
  if (w.type !== 'progress') return '';
  return [' (horizontal)', ' (vertical)', ' (arc)'][w.style ?? 0] ?? '';
}
...
async function onDeleteScene(sceneId: string) {
  if (!confirm('Delete this scene? Its widgets will be moved to Root.')) return;
...
function useTemplate(tpl: WidgetTemplate) {
  sessionStorage.setItem('widget_prefill', JSON.stringify(tpl.widget));
...
async function assignRootBg(name: string) {
  rootBgSaveError.value = '';
  const prev = rootBg.value;
  rootBg.value = name;
  try {
    await Ws.setSceneBg('', name);
  } catch (e: any) {
    rootBg.value = prev;
    rootBgSaveError.value = e?.message ?? 'Save failed';
  }
}

async function reoptimizeRoot() {
  const rootWgts  = store.items.filter((w: Widget) => !w.scene_id && w.type !== 'scene');
  const usedIcons = [...new Set(rootWgts.map((w: Widget) => (w as any).icon).filter(Boolean) as string[])];
...
/* ── table columns ── */
.type-cell { color: var(--muted); font-size: 12px; width: 110px; }
.src-cell { font-size: 11px; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.refresh-cell { font-size: 12px; width: 70px; color: var(--muted); }
